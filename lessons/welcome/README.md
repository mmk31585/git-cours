# Lesson 01 — Welcome: What Git Is (and Why We Use It)

Welcome to **Lesson 01** of the **Git & GitHub Workshop: From Zero to Hero**.
In this lesson, you will learn what Git and GitHub are, what problems they solve, and how they fit into real developer workflows.

---

## Learning Goals
By the end of this lesson, you will be able to:
- Explain what **Git** is in one sentence
- Explain what **GitHub** adds on top of Git
- Understand the basic vocabulary: **repo, commit, branch, merge, remote**
- Describe why teams rely on Git every day

---

## Simple Explanation

### What is Git?
**Git is a version control system**: a tool that tracks changes in your files over time.
It works like a time machine for your code so you can save checkpoints and return to them.

Git helps you:
- Track every change
- Recover older versions
- Experiment safely
- Work with others without overwriting each other

### What is GitHub?
**GitHub is a platform that hosts Git repositories online** and adds collaboration features like:
- Pull Requests (PRs)
- Code Reviews
- Issues (task/bug tracking)
- Discussions and Projects

> Git is the engine. GitHub is the collaboration space.

---

### With Git
- Every meaningful change becomes a **commit** (checkpoint)
- You can undo mistakes safely
- You can develop features in branches
- Teams merge work through pull requests

---

## Key Vocabulary (Quick and Practical)

| Term | Meaning (simple) |
|------|-------------------|
| Repository (repo) | The project folder and its Git history |
| Commit | A saved checkpoint of changes |
| Branch | A separate line of work (feature sandbox) |
| Merge | Combine changes from branches |
| Remote | The online copy of your repo (often GitHub) |

---

## Practice Exercise
Complete the lesson practice in:

- [Exercise: Lesson 01 Task](../../exercises/welcome/task.md)

---

## Navigation
- Next: [Lesson 02 — Installing Git + First-Time Setup](../setup/README.md)
